...
def test_deploy_charm_and_tempo(juju: jubilant.Juju, charm: pathlib.Path) -> None:
    ...
    # THEN Tempo goes active. The charm stays blocked (no metrics provider/consumer),
    # which is expected and doesn't stop its hooks (and traces) from running.
    juju.wait(
...
def test_charm_traces_reach_tempo(juju: jubilant.Juju) -> None:
    ...
    services = get_ingested_traces_tag_values(
        get_app_ip_address(juju, TEMPO), tag="service.name"
    )
