...
def test_charm_active_with_charm_tracing_relation(
    context,
    metrics_provider_relation,
    metrics_consumer_relation,
    charm_tracing_relation,
    tls_charm_tracing_relation,
    ca_cert_relation,
    tls,
):
    """The charm reaches active with a tracing relation present, with or without TLS."""
    # GIVEN a fully related charm whose charm-tracing backend is HTTP or HTTPS
    tracing_relation = tls_charm_tracing_relation if tls else charm_tracing_relation
    relations = [
        metrics_provider_relation,
        metrics_consumer_relation,
        tracing_relation,
    ] + ([ca_cert_relation] if tls else [])
    state_in = testing.State(leader=True, relations=relations)
...
def test_charm_publishes_tracing_receivers_on_relation_changed(context, charm_tracing_relation):
    ...
    state_in = testing.State(leader=True, relations=[charm_tracing_relation])

    # WHEN the tracing relation data changes
    state_out = context.run(context.on.relation_changed(charm_tracing_relation), state_in)
...
def test_charm_withdraws_tracing_receivers_on_relation_broken(context, charm_tracing_relation):
